...
from blivedm.blive import BLiveClient
from blivedm import models, handlers
...
def post_batch():
    ...
    if BATCH:
        try:
            requests.post(SERVER + PATH, json={'names': list(BATCH)})
        except Exception:
            pass
        BATCH.clear()
    LAST_POST = time.time()
...
if __name__ == '__main__':
    ...
    client.set_handler(H())
    client.start()
    while True:
        if time.time() - LAST_POST > 2:
            post_batch()
        time.sleep(0.2)
