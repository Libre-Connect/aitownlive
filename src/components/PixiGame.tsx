...
export const PixiGame = (props: {
  ...
  useEffect(() => {
    ...
    const centers = () => {
      ...
      for (const p of props.game.world.players.values()) {
        const act = (p as any).activity;
        if (act && act.until > Date.now() && (act.description === 'build' || act.description === 'explore')) {
          const cx = p.position.x * tileDim;
          const cy = p.position.y * tileDim;
          const x = clamp(cx, margin.x, worldPx.w - margin.x);
          const y = clamp(cy, margin.y, worldPx.h - margin.y);
          out.push(new PIXI.Point(x, y));
        }
